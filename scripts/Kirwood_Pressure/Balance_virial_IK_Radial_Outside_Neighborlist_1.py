...
# Read the argument (frame index ) passed from the Bash script 
...
class Radius_Calculation:
    ...
    def Radius(self):
        ...
        # box_volume_outside=volume of the box−volume of the large cylinder
...
class InitializeSimulation:
    ...
    def calculate_pressure(self):
        ...
        pressures_ATM = [] 
        neighbor_lists = []
        output_file = f"atom_positions_and_neighbors_Outside_{self.segment}_data.txt"
        # Open the file in append mode
        with open(output_file, "a") as f:
            Indices = (self.indices)
            # Keep track of processed pairs to avoid double counting
            processed_pairs = set()

            for index, row in df_neighbors.iloc[:-1].iterrows():
                selected_atom = row['selected_index']
                neighbors = row['neighbor_indices']
                
                # Remove self-neighboring atoms
                neighbors = [neighbor for neighbor in neighbors if neighbor != selected_atom]
                
                # Process neighbors
                filtered_neighbors = []
                for neighbor in neighbors:
                    # If the reverse pair (neighbor, selected_atom) has already been processed, skip it
                    if (selected_atom, neighbor) not in processed_pairs and (neighbor, selected_atom) not in processed_pairs:
                        filtered_neighbors.append(neighbor)
                        # Mark the pair as processed
                        processed_pairs.add((selected_atom, neighbor))
                
                #print(f"Atom {selected_atom} has neighbors: {filtered_neighbors}")
                
                
                neighbor_lists.append(filtered_neighbors)

                # Get positions of the current atom and its neighbors 
                current_atom_position = self.universe[self.indices[index]].position
                #print("current_atom_position:\n",current_atom_position)
                f.write(f"current_atom_position: {self.universe[self.indices[index]].position}\n")
                #print("current_atom_position in class:",current_atom_position)
                # Get positions of neighbors
                #neighbor_positions = [self.universe.atoms[Indices[neighbor]].position.tolist() for neighbor in filtered_neighbors]
                neighbor_positions = [self.universe[self.indices[ele]].position for ele in filtered_neighbors]
                #print("neighbor_positions:\n",neighbor_positions)
                f.write(f"neighbor_positions: {neighbor_positions}\n\n")
        ...
        CONVERSION_TO_BAR = 69475.55667#4184*16.6057 
        CONVERSION_TO_ATM = 68567.392  #4184*16.388
        ...
        Sigma_Virial_kCal_per_mol= 0.5*(Sigma_Virial)
        #print("Virial in kCal/mol:",Sigma_Virial_kCal_per_mol)
        Sigma_Virial_kJ_per_mol= 0.5*(Sigma_Virial*4.184)
        #print("Virial in kJ/mol (Consistent with GROMACS output when (Vir-XX+Vir-YY+Vir-ZZ):",Sigma_Virial_kJ_per_mol)
        ...
        Pressure_BAR = ( (np.sum(total_virials_all_particles)) / (3*self.box_volume_outside)) * CONVERSION_TO_BAR  #  bar 
        #print(f"Pressure in My code in Bar: {Pressure_BAR:.10f}")
        Pressure_ATM = ( (np.sum(total_virials_all_particles)) / (3*self.box_volume_outside)) * CONVERSION_TO_ATM  #  atm 
        #print(f"Pressure in My code in atm: {Pressure_ATM:.10f}")
        Virial_KJ_MOL.append(Sigma_Virial_kJ_per_mol)
        pressures_BAR.append(Pressure_BAR)
        pressures_ATM.append(Pressure_ATM)
        
        return Virial_KJ_MOL , pressures_BAR , pressures_ATM
...
#for ts, ts2 in zip(universe.trajectory[:1], universe2.trajectory[:1]): 
    ...
    for segment in range(num_segments):
        ...
        cutoff=12
        # inner_radius+cutoff
        DONOT_CHANGE = 1000000000
        selected_atoms_inside = segment_ag[[np.linalg.norm(atom.position[1:3] - center_of_mass[1:3]) < DONOT_CHANGE  for atom in segment_ag]]
        ...
        # Extract X, Y, Z coordinates
        x = selected_atoms_inside_ag_positions[:, 0]
        y = selected_atoms_inside_ag_positions[:, 1]
        z = selected_atoms_inside_ag_positions[:, 2]
        # Create a 3D plot
        fig = plt.figure(figsize=(8, 6))
        ax = fig.add_subplot(111, projection='3d')
        theta = np.linspace(0, 2 * np.pi, 100) 
        ...
        plt.savefig(filename, dpi=300, bbox_inches='tight')
        #plt.show()
        print(f"# of particles Outside cutoff: {len(selected_atoms_inside)} atoms")
        #from MDAnalysis.analysis import distances
...
#         # Run FastNS
#         box = np.array([1e6, 1e6, 1e6, 90.0, 90.0, 90.0])
#         ns = FastNS(cut_off, selected_atoms_inside_positions, box=box)
#         results = ns.self_search()
#         neighbor_list = results.get_pairs()
#         sorted_neighbor_list = neighbor_list[np.argsort(neighbor_list[:, 0])]
#         print("neighbor_list:\n",sorted_neighbor_list)
#         #print("Shape of neighbor list:", neighbor_list.shape)
#         
#         
#         # Extract neighbors of atom 0
#         neighbors_of_atom_0 = sorted_neighbor_list[sorted_neighbor_list[:, 0] == 0][:, 1]
#         #print(neighbors_of_atom_0)
# 
#         # Save to text file
#         with open("neighbors_atom_0.txt", "w") as f:
#             for neighbor in neighbors_of_atom_0:
#                 f.write(f"{neighbor}\n")
#                 
#                 
        
        
#         with open("neighbor_list_output.txt", "w") as f:
...
#                 
        ...
        selected_pos = ag_water.positions[selected_atom_inside_indices]  #  atoms of interest
        #print("selected_pos:\n",(selected_pos))
        all_pos = ag_water.positions                                     #  potential neighbors 
        #print("all_pos:\n",(all_pos))
                
                
        print("BEFORE NEIGHBOR LIST")

        # Compute capped distances (only pairs within cutoff, no distances)
        ...
        # Convert to (selected_index -> list of neighbor_indices)
        neighbor_dict = defaultdict(list)
        for i, j in zip(*result) if isinstance(result, tuple) else result:
            neighbor_dict[i].append(j)

        # Convert to DataFrame
        df_neighbors = pd.DataFrame([
            {'selected_index': k, 'neighbor_indices': v}
            for k, v in neighbor_dict.items()
        ])
        
        print("Memory after slicing (MiB):", df_neighbors.memory_usage(deep=True).sum() / 1024**2)
        print("AFTER NEIGHBOR LIST")

        
        df_neighbors.to_csv('neighbors_list_Outside.txt', index=False, sep='\t')
                        

#         # Group neighbors by atom index in selected_pos
#         neighbor_dict = defaultdict(list)
#         for i, j in result:
#             neighbor_dict[i].append(j)
# 
...
#             for atom_idx in sorted(neighbor_dict.keys()):
#                 neighbors = neighbor_dict[atom_idx]
#                 neighbor_str = " ".join(map(str, neighbors))
#                 f.write(f"Atom {atom_idx}: {neighbor_str}\n")
#                     
#                     
...
#         
        ...
        My_Virial_KJ_MOL.append(pressure[0])
        My_pressures_BAR.append(pressure[1])
        My_pressures_ATM.append(pressure[2])
