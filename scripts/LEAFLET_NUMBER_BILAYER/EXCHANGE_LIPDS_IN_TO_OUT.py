...
def count_lipids_and_transfer(universe, lipid_resnames=["DLPC"], headgroup_bead="PO4", tailgroup_bead="C3B", n_to_transfer=50):
    ...
    for resid in inner_resids_all:
        ...
        if 100.0 < com_x < 600.0:
            inner_resids_filtered.append(resid)

    if len(inner_resids_filtered) < n_to_transfer:
        raise ValueError(f"Only {len(inner_resids_filtered)} inner lipids in 10 < x < 60 nm range; cannot move {n_to_transfer}.")
    ...
    print(f"✅ Transferred and mirrored {n_to_transfer} lipids from inner to outer leaflet in 0 < x < 70 nm.")
